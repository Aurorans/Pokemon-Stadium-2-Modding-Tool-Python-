...
for root, dirs, files in os.walk(project_root):
    for f in files:
        if f.lower() == 'prime_cup_rental_pokemon.txt':
...
if not RENTAL_FILE:
    print("Could not find 'prime_cup_rental_pokemon.txt' in the parent or its subfolders.")
    print("Please make sure the file exists and try again.")
    sys.exit(1)  # Stops the script safely right here    
...
def execute_modding_resource():
    ...
    for entry in config_entries:
        try:
            ...
            base = PRIME_CUP_START + (slot_idx * POKEMON_SIZE)

            # --- APPLY BYTE WRITING ---
            rom_bytes[base + 0] = 0x64  # Level 100
